...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == SVProgressHUDAnimationTypeFlat){
        ...
        indefiniteAnimatedView.adjustsStrokeForTraitCollection = (_defaultStyle == SVProgressHUDStyleAutoLight)
        || (_defaultStyle == SVProgressHUDStyleAutoDark);
...
- (SVProgressAnimatedView*)ringView {
    ...
    // Update styling
    _ringView.adjustsStrokeForTraitCollection = (_defaultStyle == SVProgressHUDStyleAutoLight) || (_defaultStyle == SVProgressHUDStyleAutoDark);
...
- (UIColor*)foregroundColorForStyle {
    switch(_defaultStyle) {
        case SVProgressHUDStyleLight:
            return [UIColor blackColor];
        case SVProgressHUDStyleDark:
            return [UIColor whiteColor];
        case SVProgressHUDStyleCustom:
            return self.foregroundColor;
        case SVProgressHUDStyleAutoLight:
            if (@available(iOS 13.0, *)) {
                return [UIColor labelColor];
            } else {
                return [UIColor blackColor];
            }
        case SVProgressHUDStyleAutoDark:
            if (@available(iOS 13.0, *)) {
                return [UIColor labelColor];
            } else {
                return [UIColor whiteColor];
...
- (void)fadeInEffects {
    if(_defaultStyle != SVProgressHUDStyleCustom) {
        // Add blur effect
        ...
        CGFloat alphaComponent = 0.6;
        switch(_defaultStyle) {
            case SVProgressHUDStyleLight:
                blurEffectStyle = UIBlurEffectStyleLight;
                blurEffect = [UIBlurEffect effectWithStyle:blurEffectStyle];
                self.hudView.backgroundColor = [self.backgroundColorForStyle colorWithAlphaComponent:alphaComponent];
                break;
            case SVProgressHUDStyleAutoLight:
                if (@available(iOS 13.0, *)) {
                    blurEffectStyle = (self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleLight) ? UIBlurEffectStyleLight : UIBlurEffectStyleDark;
                } else {
                    blurEffectStyle = UIBlurEffectStyleLight;
                }
                blurEffect = [UIBlurEffect effectWithStyle:blurEffectStyle];
            ...
            case SVProgressHUDStyleAutoDark:
                if (@available(iOS 13.0, *)) {
                    blurEffectStyle = (self.traitCollection.userInterfaceStyle == UIUserInterfaceStyleDark)
                ...
                break;
            case SVProgressHUDStyleCustom:
                blurEffect = self.hudViewCustomBlurEffect;
